...
- Dependency trust boundary: `CLAUDE.md` → "Pinning policy"; `pyproject.toml` per-dep comments carry the per-library justification. A future `THREAT_MODEL_DEPENDENCIES.md` is the right home for the threat-model framing of the vendored boundary.
