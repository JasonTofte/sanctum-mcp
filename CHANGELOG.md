...
- `docs/figures/architecture_flow.png`: the two ASCII flows in
  `docs/ARCHITECTURE.md` (tool call and finding) rendered as a dark, brand-coloured
  image for the Devpost gallery and slides. Generated by the new
  `scripts/render_arch_diagram.py`, which parses the doc's fenced blocks so the
  image stays in sync with the source. No new runtime dependency (matplotlib is
  already a `[dev]` extra for the eval figures).

### Changed — NIST dataset doc marked complete (2026-06-14)

- `docs/DATASET_NIST_DATALEAKAGE.md`: removed stale `STATUS: DRAFT` /
  `PENDING OPERATOR RUN` scaffolding from a doc whose results were already filled in
  (operator run + per-segment SHA-1 verification completed 2026-06-13). The trailing
  `[STAGED for ACCURACY.md]` stub was deleted — that section now lives in
  `docs/ACCURACY.md`; the dataset doc links to it instead. No results or claims changed.
