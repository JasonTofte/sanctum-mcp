...
2. **All tool output is quarantined.** Every MCP tool must return its payload wrapped in `<evidence-untrusted>…</evidence-untrusted>` after passing through `sanctum.sanitize.strip_known_injection_patterns()`. No unstructured free-text returns. **Error-channel corollary**: FastMCP's `isError` channel serializes raised exception strings to the LLM *without* the success-path `sanitize()` and `<evidence-untrusted>` wrapper. Every raise site that interpolates attacker-influenceable input therefore scrubs that input via `sanctum.parsers._fixture_io._safe_field()`, which substitutes the same delimiter inventory (`<`, `>`, `\x00`–`\x1f`, plus `INVISIBLE_CODEPOINT_CLASS`) that the parser-boundary reject pattern uses — one shared inventory between the success path and the error path. See [`docs/THREAT_MODEL_SANITIZATION.md`](docs/THREAT_MODEL_SANITIZATION.md) §"Secondary error-message channel".

3. **`claim_finding` cites `audit_ids[]`; the gate refuses unresolved citations.** Every tool call produces an `audit_id` (UUID), written to the append-only JSONL ledger before the call returns. The server rejects `claim_finding` calls where any `audit_id` is missing or fails to resolve to a ledger entry — eliminating the LLM-fabricated-citation failure mode (the agent cannot invent an audit_id to satisfy the family-corroboration gate). The ledger is HMAC-SHA-256-chained via `SANCTUM_LEDGER_HMAC_KEY` so an attacker with ledger write access cannot silently rewrite a citation's underlying entry; the server refuses to start if the key is unset (no silent downgrade to plain SHA-256). Optional RFC 3161 TSA stamping via `sanctum.notary.stamp_head()` extends forgery-detection across HMAC-key compromise; see [`docs/THREAT_MODEL_LEDGER.md`](docs/THREAT_MODEL_LEDGER.md).
