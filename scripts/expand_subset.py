...
Selection criteria (also enforced by the Jaccard test):
  1. Question is answerable from exactly one of the 5 Sanctum artifact
     families.
  2. Family tag is verifiable from the artifact description alone (no
     cross-family inference needed).
  3. ``scoring_pattern`` achievable without verbatim copy of the
     upstream answer.
  4. ``Jaccard(justification, upstream_question_text) < 0.30``.
